	...
	switch (SCREEN){
		...
		case MATERIAL:
			...
			if (MATERIAL_PAGE == 0){
				...
				cmd_button(215, 190, 100, 40, R_FONT, 0, "\x25\x1a\x0c\x0a\x51"); //След
				cmd(TAG_MASK(0));
			}
				...
				cmd_button(110, 190, 100, 40, R_FONT, 0, "\x21\x24\x0c\x0a\x51"); //Пред
				cmd(TAG_MASK(0));
			} else {
				cmd(TAG_MASK(1));
				cmd(TAG(98));          // assign the tag value
			...
			cmd_text(5, 7, R_FONT, 0, "\x27\x14\x22"); //Тип
			cmd_text(110, 7, R_FONT, 0, "\x13\x1c\x42\x62"); //Имя?
			...
			char *MATERIAL_NAMES[MATERIAL_SIZE] = {
					"\x25\x28\x02\x1a\x14", // Стали
					// \x54 - .  /   \x26\x22\x1a\x81 - спл.
					"\x01\x1a\x40\x1c\x51\x26\x22\x1a\x51", // Алюм.спл.
					"\x27\x14\x28\x02\x1e\x51\x26\x22\x1a\x51", // Титан.спл.
					"\x1b\x02\x08\x1e\x51\x26\x22\x1a\x51", // Магн.спл.
					"\x1b\x0c\x0a\x1e\x51\x26\x22\x1a\x51", // Медн.спл.
					"\x1d\x14\x18\x0c\x1a\x51\x26\x22\x1a\x51", // Никел.спл.
					"\x21\x20\x1a\x3c\x12\x20\x06\x51\x54", // Пользов.1
					"\x21\x20\x1a\x3c\x12\x20\x06\x51\x55", // Пользов.2
					"\x21\x20\x1a\x3c\x12\x20\x06\x51\x56", // Пользов.3
					"\x21\x20\x1a\x3c\x12\x20\x06\x51\x57",// Пользов.4
			};
