...
void ft800memWrite8__(unsigned char ftData8)
{
	HAL_GPIO_WritePin(GPIOB, FT800_CS_N, GPIO_PIN_RESET);	// Set chip select low
...
void ft800memWrite16__(unsigned long ftAddress, unsigned int ftData16)
{
	unsigned char cTempData[2];														// 16-bit data to write
...
void ft800memWrite32__(unsigned long ftData32)
{
	unsigned char cTempData[4];												// 32-bit data to write
	...
	for (int j = 0; j < 4; j++)						// Start with least significant byte
	{
		HAL_SPI_Transmit(&hspi1, &cTempData[j], 1, TIMEOUTSPI);			// Send SPI byte
	}
...
ft_uint8_t ft800memRead8__(ft_uint32_t ftAddress)
{
	ft_uint32_t ftData32;															// FT800 Memory Address
	ft_uint8_t cTempData;														// Place holder for 32-bits being read
	ft_uint8_t cZeroFill;															// Dummy byte

	HAL_GPIO_WritePin(GPIOB, FT800_CS_N, GPIO_PIN_RESET);	// Set chip select low

	HAL_SPI_Receive(&hspi1, &cTempData, 1, TIMEOUTSPI);				// Receive data byte

	HAL_GPIO_WritePin(GPIOB, FT800_CS_N, GPIO_PIN_SET);		// Set chip select high

  return cTempData;																									// Return 8-bits
}

ft_uint16_t ft800memRead16__(ft_uint32_t ftAddress)
{
	ft_uint16_t ftData16;															// FT800 Memory Address
	ft_uint8_t cTempData[2];														// Place holder for 32-bits being read
	...
	ftData16 = ( (cTempData[1]<< 8) |
						 (cTempData[0])); 														// Low byte

	return ftData16;
}

ft_uint32_t ft800memRead32__(ft_uint32_t ftAddress)
{
	ft_uint32_t ftData32;															// FT800 Memory Address
	ft_uint8_t cTempData[4];														// Place holder for 32-bits being read
...
ft_uint8_t ft800memWriteRead8__(ft_uint8_t value)
{
	ft_uint8_t ftData8 = ZERO;

	HAL_GPIO_WritePin(GPIOB, FT800_CS_N, GPIO_PIN_RESET);	// Set chip select low

	HAL_SPI_TransmitReceive(&hspi1, &value, &ftData8, 1, TIMEOUTSPI);						// Receive data byte
